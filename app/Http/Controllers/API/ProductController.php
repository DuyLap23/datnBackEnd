    ...
    public function store(ProductRequest $request)
    {
        ...
        if ($request->has('product_variants')) {
            foreach ($request->product_variants as $key => $value) {
                ...
                // Kiểm tra số lượng phần tử trong mảng $tmp
                if (count($tmp) < 2) {
                    // Nếu không đủ phần tử, ghi log và tiếp tục với biến thể tiếp theo
                    Log::error("Invalid product variant key: $key");
                    continue;
                }
            ...
            return response()->json($product->load(['category', 'brand', 'tags', 'productImages', 'productVariants.productColor', 'productVariants.productSize']), 201);
        } catch (\Exception $e) {
            DB::rollBack();
 ...
 */
    ...
    public function update(Request $request, Product $product)
    {
        ...
        try {
            ...
            $dataProduct['is_new'] = $request->input('is_new', 0);
            $dataProduct['is_show_home'] = $request->input('is_show_home', 0);
            $dataProduct['slug'] = Str::slug($dataProduct['name']) . '-' . $product->sku;
